...
'''---------     
def process(length, is_doc, in_list, out_list):
    get_docs = []
    run = 0
    for doc in is_doc:
        print(run)
        ...
        for check in in_list:
            if type(check) == str:
                slide_in_list.append([m.start() for m in re.finditer(check,wiki[102278][3])])
            else:
                f_word = [m.start() for m in re.finditer(check[0],wiki[102278][3])]
                s_word = [m.start() for m in re.finditer(check[1],wiki[102278][3])]
                c_word = []
                for i in f_word:
                    for j in s_word:
                        if abs(i-j) < check[2]:
        ...
        for check in out_list:
            if type(check) == str:
                slide_out_list.append([m.start() for m in re.finditer(check,wiki[102278][3])])
            else:
                f_word = [m.start() for m in re.finditer(check[0],wiki[102278][3])]
                s_word = [m.start() for m in re.finditer(check[1],wiki[102278][3])]
                c_word = []
...
#def slide_window(length, is_doc, in_list, out_list):
#    get_docs = []
#    run = 0
#    for doc in is_doc:
#        print(run)
...
#        for check in out_list:
#            if type(check) == str:
#                slide_out_list.append([m.start() for m in re.finditer(check,wiki[202][3])])
#            else:
#                f_word = [m.start() for m in re.finditer(check[0],wiki[doc][3])]
#                s_word = [m.start() for m in re.finditer(check[1],wiki[doc][3])]
#                c_word = []
...
#        else:
#            slide_doc = np.zeros((len(in_list)+len(out_list),end-start+1), dtype = bool)
#            slide_doc2 = np.zeros((len(in_list)+len(out_list),end-start+1-length+1), dtype = bool)
#            iter = 0
#            for sublist in slide_in_list:
#                for num in sublist:
#                    slide_doc[iter,num-start] = True
#                count = 0
#                while count < end-start+1-length+1:
#                    slide_doc2[iter,count] = True in slide_doc[iter,count:count+50]
#                    count += 1
#                iter += 1        
#            for sublist in slide_out_list:
#                for num in sublist:
...
#                count = 0
#                while count < end-start+1-length+1:
#                    slide_doc2[iter,count] = not(True in slide_doc[iter,count:count+50])
#                    count += 1
#                iter += 1
#            entre = np.where(slide_doc2.sum(axis=1) == min(slide_doc2.sum(axis=1)))[0][0]
#            for i in np.where(slide_doc2[entre,:] == True)[0]:
#                if not(False in slide_doc2[:,i]):
#                    get_docs.append(doc)
#                    break
#        run+=1
#    return get_docs
...
t0 = time.time()
#in_list, out_list = in_out_list(check_word)
